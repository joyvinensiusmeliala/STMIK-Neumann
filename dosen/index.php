...
$carimapel = mysqli_query($con,"SELECT * FROM tb_silabus 
	...
	WHERE tb_silabus.id_silabus='$jadwal[id_silabus]'
	");
...
"></i>
							...
							<div class="collapse" id="sidebarLayouts">
								<ul class="nav nav-collapse">
									...
									foreach ($mengajar as $dm) { 
										$carimpelajaran = mysqli_query($con,"SELECT * FROM tb_silabus 
										INNER JOIN tb_master_mapel ON tb_silabus.id_mapel=tb_master_mapel.id_mapel
										INNER JOIN tb_semester ON tb_silabus.id_semester=tb_semester.id_semester
										INNER JOIN tb_kurikulum ON tb_silabus.id_kurikulum=tb_kurikulum.id_kurikulum
										WHERE tb_silabus.id_silabus='$dm[id_silabus]'
										");
										
										foreach ($carimpelajaran as $cmpela) 	
									?>
									<li>
										<a href="?page=absen&pelajaran=<?=$dm['id_mengajar'] ?> ">
											<span class="sub-item"> <?=($dm['nama_kelas']); ?> - <b><?=strtoupper($cmpela['mapel']); ?></b></span>
										</a>
									</li>
								<?php } ?>
								</ul>
							...
							<div class="collapse" id="rekapAbsen">
								<ul class="nav nav-collapse">
									...
									foreach ($mengajar as $dm) {
										$carimpel = mysqli_query($con,"SELECT * FROM tb_silabus 
										INNER JOIN tb_master_mapel ON tb_silabus.id_mapel=tb_master_mapel.id_mapel
										INNER JOIN tb_semester ON tb_silabus.id_semester=tb_semester.id_semester
										INNER JOIN tb_kurikulum ON tb_silabus.id_kurikulum=tb_kurikulum.id_kurikulum
										WHERE tb_silabus.id_silabus='$dm[id_silabus]'
										...
										foreach ($carimpel as $cmpel) 	
									?>
									<li>
										<a href="?page=rekap&pelajaran=<?=$dm['id_mengajar'] ?> ">
										<span class="sub-item"> <?=($dm['nama_kelas']); ?> - <b><?=strtoupper($cmpel['mapel']); ?></b></span>
										</a>
									</li>
								<?php } ?>
								</ul>
							...
							<!-- <div class="collapse" id="rekapAbsen">
								...
								<?php 
									...
									foreach ($kelas as $k) {?>
									<li>
										<a href="?page=rekap&pelajaran=<?=$dm['id_mkelas'] ?> ">
										<span class="sub-item"> <?=strtoupper($dm['nama_kelas']); ?> </span>
										</a>
									</li>
								<?php } ?>
								</ul>
		...
		<div class="main-panel">
			<div class="content">
				...
				if ($page=='absen') {
					...
					}elseif ($act=='absen_bulan') {
						include 'modul/absen/absen_bulan.php';
					}elseif ($act=='view') {
						include 'modul/absen/view_absen.php';
					}elseif ($act=='set_absen') {
						include 'modul/absen/set_absen.php';
					}elseif ($act=='set_absen_tolak') {
						include 'modul/absen/set_absen_tolak.php';
					}			
				}elseif ($page=='rekap') {
					if ($act=='') {
						include 'modul/rekap/rekap_absen.php';
					}elseif ($act=='view') {
						include 'modul/rekap/view_absen.php';
					}elseif ($act=='set_absen_terima') {
						include 'modul/rekap/set_absen.php';
					}elseif ($act=='set_absen_tolak') {
						include 'modul/rekap/set_absen_tolak.php';
					}			
				}elseif ($page=='jadwal') {
					if ($act=='') {
